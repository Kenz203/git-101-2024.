...
A.k.a. summary functions :turn a possibly long vector into a single value (still vector!)
...
remark %>% and |> are rooughy the same
```{r}
white_wine|>
  summarise(
...
white_wine |>
  ...
  mutate(a_m_median = alcohol -median(alcohol),
         .keep="none") #default behavior of mutate is to add new column and add previous columns, so keep none is don't keeep other columns
```
...
```{r}
white_wine|>
  reframe(quantile(`residual sugar`),  #reframe is the official solution
          mean(`residual sugar`))
```
```{r}
...
```
We want to compute the quantile
```{r}
white_wine|>
  pull(alcohol)|>
...
white_wine|>
  ...
  geom_bar()
```
 difference between the fist. graphical is that:the second class is very small, we have clear alcohol prime effect (Strong variation). It shows the correlation between quality and alcohol class better.
...
white_wine|>
  ...
  select(1:2)                      #first 2 columns (and quality)
```
stratified sampling is very different, it make sense to some settings so stratify ( fromastatistical point ofview). and not for others
```{r}
white_wine|>
...
diamonds |> 
  ...
  summarise(median(price)) #number of group = product of the number of group of cut & clarity
```
```{r}
diamonds |> 
  group_by(cut, clarity)|> 
...
```{r}
diamonds|> 
  group_by(cut,color) |>  #we group cut & color
  summarise(N=n(), .group="drop_last") |> #count how many number do we have
  mutate(prob=N/sum(N)) |>
  ungroup()  # ungroup removes groups, 

```
```{r}
diamonds|> 
  summarise(N=n(), .by=c(cut,color)) |> #count how many number do we have
  mutate(prob=N/sum(N)) |>
  arrange(cut,color)
```
...
## tidying data
Dataset too long or too wide -> pivoting
```{r}
library(tidyr)
```
